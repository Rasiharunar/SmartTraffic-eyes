...
class ScreenVehicleCounter:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("YOLO Vehicle Counter - Screen Capture v2.3 DIRECTIONAL")
        ...
        # Single counting line with direction detection
        self.counting_line = None  # Only one line: [p1, p2]
        ...
        # Tracking with direction detection
        self.tracked_vehicles = {}
        self.next_id = 0
        self.counted_ids = set()
        
        # Directional counting
        self.vehicle_count_up = defaultdict(int)    # Going UP/LEFT direction
        self.vehicle_count_down = defaultdict(int)  # Going DOWN/RIGHT direction
    ...
    def _init_database(self):
        ...
        try:
            ...
            ''')
            self.db_conn.commit()
            print("Database 'deteksi_kendaraan_directional' (PostgreSQL) initialized successfully.")
    ...
    def save_counts_to_db(self):
        """Saves the current vehicle counts to the database manually."""
        if not self.db_conn:
        ...
        motor_up = self.vehicle_count_up['motorcycle']
        motor_down = self.vehicle_count_down['motorcycle']
        mobil_up = self.vehicle_count_up['car']
        mobil_down = self.vehicle_count_down['car']
        truk_up = self.vehicle_count_up['truck']
        truk_down = self.vehicle_count_down['truck']
        bus_up = self.vehicle_count_up['bus']
        bus_down = self.vehicle_count_down['bus']
        ...
        try:
            ...
            ''', (motor_up, motor_down, mobil_up, mobil_down, truk_up, truk_down, 
                  bus_up, bus_down, self.total_count_up, self.total_count_down, 
                  session_name, current_time))
            self.db_conn.commit()
            
            messagebox.showinfo("Database Save", 
                               f"Successfully saved counts to database!\n"
                               f"Session: {session_name}\n"
                               f"Total UP: {self.total_count_up}\n"
                               f"Total DOWN: {self.total_count_down}\n"
                               f"Time: {current_time}")
            
            print(f"Saved directional counts to DB: Session={session_name}, "
                  f"UP={self.total_count_up}, DOWN={self.total_count_down} at {current_time}")
                  
        except psycopg2.Error as e:
            messagebox.showerror("Database Error", f"Error saving counts to PostgreSQL database: {e}")
    ...
    def setup_gui(self):
        ...
        # Database controls
        db_group = ttk.LabelFrame(control_frame, text="Database", padding=10)
        db_group.pack(side=tk.LEFT, padx=(0, 10))

        ttk.Button(db_group, text="Store to Database", command=self.save_counts_to_db).pack(side=tk.LEFT, padx=5)
        ...
        # Instructions
        self.instructions = ttk.Label(self.video_frame, text="1. Select region → 2. Preview will start → 3. Draw ONE line → 4. Start detection", 
                                 font=("Arial", 9))
        ...
        # Statistics frame
        stats_frame = ttk.LabelFrame(content_frame, text="Directional Vehicle Statistics", padding=10)
        ...
        # Total counts
        self.total_up_label = ttk.Label(count_frame, text="Total UP: 0", font=("Arial", 14, "bold"), foreground="green")
        self.total_up_label.pack(pady=2)
        
        self.total_down_label = ttk.Label(count_frame, text="Total DOWN: 0", font=("Arial", 14, "bold"), foreground="red")
        self.total_down_label.pack(pady=2)

        ttk.Separator(count_frame, orient='horizontal').pack(fill=tk.X, pady=10)

        # Directional counts by vehicle type
        # Cars
        car_frame = ttk.Frame(count_frame)
        car_frame.pack(fill=tk.X, pady=2)
        ttk.Label(car_frame, text="Cars:", font=("Arial", 10, "bold")).pack(side=tk.LEFT)
        self.car_up_label = ttk.Label(car_frame, text="↑0", font=("Arial", 10), foreground="green")
        self.car_up_label.pack(side=tk.RIGHT, padx=5)
        self.car_down_label = ttk.Label(car_frame, text="↓0", font=("Arial", 10), foreground="red")
        self.car_down_label.pack(side=tk.RIGHT, padx=5)

        # Motorcycles
        moto_frame = ttk.Frame(count_frame)
        moto_frame.pack(fill=tk.X, pady=2)
        ttk.Label(moto_frame, text="Motorcycles:", font=("Arial", 10, "bold")).pack(side=tk.LEFT)
        self.moto_up_label = ttk.Label(moto_frame, text="↑0", font=("Arial", 10), foreground="green")
        self.moto_up_label.pack(side=tk.RIGHT, padx=5)
        self.moto_down_label = ttk.Label(moto_frame, text="↓0", font=("Arial", 10), foreground="red")
        self.moto_down_label.pack(side=tk.RIGHT, padx=5)

        # Buses
        bus_frame = ttk.Frame(count_frame)
        bus_frame.pack(fill=tk.X, pady=2)
        ttk.Label(bus_frame, text="Buses:", font=("Arial", 10, "bold")).pack(side=tk.LEFT)
        self.bus_up_label = ttk.Label(bus_frame, text="↑0", font=("Arial", 10), foreground="green")
        self.bus_up_label.pack(side=tk.RIGHT, padx=5)
        self.bus_down_label = ttk.Label(bus_frame, text="↓0", font=("Arial", 10), foreground="red")
        self.bus_down_label.pack(side=tk.RIGHT, padx=5)

        # Trucks
        truck_frame = ttk.Frame(count_frame)
        truck_frame.pack(fill=tk.X, pady=2)
        ttk.Label(truck_frame, text="Trucks:", font=("Arial", 10, "bold")).pack(side=tk.LEFT)
        self.truck_up_label = ttk.Label(truck_frame, text="↑0", font=("Arial", 10), foreground="green")
        self.truck_up_label.pack(side=tk.RIGHT, padx=5)
        self.truck_down_label = ttk.Label(truck_frame, text="↓0", font=("Arial", 10), foreground="red")
        self.truck_down_label.pack(side=tk.RIGHT, padx=5)
        ...
        # Direction info
        direction_frame = ttk.LabelFrame(stats_frame, text="Direction Legend", padding=5)
        direction_frame.pack(fill=tk.X, pady=10)

        ttk.Label(direction_frame, text="↑ UP/LEFT direction", font=("Arial", 8), foreground="green").pack(anchor=tk.W)
        ttk.Label(direction_frame, text="↓ DOWN/RIGHT direction", font=("Arial", 8), foreground="red").pack(anchor=tk.W)
    ...
    def toggle_preview(self):
        ...
        if self.is_previewing:
            ...
            self.status_label.config(text="Preview active - Draw your single counting line")
            self.instructions.config(text="Preview active! Draw ONE counting line for directional detection.")
    ...
    def update_line_info_display(self):
        ...
        pass  # Can add line info display if needed
    ...
    def enable_line_drawing(self):
        ...
        self.instructions.config(text="LINE DRAWING ENABLED: Click and drag on the video to create ONE counting line.")
    ...
    def toggle_capture(self):
        ...
        if self.is_capturing:
            ...
            self.status_label.config(text="Detection active - Counting vehicles with direction...")
    ...
    def capture_loop(self):
        ...
        while self.is_capturing:
            try:
                ...
                # YOLO detection with multiple scales for better accuracy
                results = self.model(frame, verbose=False, conf=0.05, iou=0.5)
                ...
                for r in results:
                    ...
                    if boxes is not None:
                        for box in boxes:
                            ...
                            if cls in self.vehicle_classes and conf > 0.2:  # Increased from 0.1
                                x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
                                
                                # Filter out very small detections that might be noise
                                width = x2 - x1
                                height = y2 - y1
                                if width > 10 and height > 10:  # Minimum size filter
                                    detections.append({
                ...
                self.check_line_crossings_directional()
    ...
    def update_tracking(self, detections):
        """Update vehicle tracking - IMPROVED VERSION"""
        max_distance = 200  # Increased from 100
        updated_tracks = {}
        ...
        for detection in detections:
            ...
            for track_id, track in self.tracked_vehicles.items():
                if track['class'] == detection['class']:
                    ...
                    if len(track['path']) >= 2:
                        ...
                        # Use the smaller distance for better matching
            ...
            if best_match is not None:
                # Keep more path history for better tracking
                path_history = self.tracked_vehicles[best_match]['path']
                if len(path_history) > 10:  # Keep last 10 positions
                ...
                updated_tracks[best_match] = {
                    ...
                    'confidence': detection['confidence']  # Track confidence
                }
            else:
        ...
        # Remove old tracks - increased timeout
        current_time = time.time()
        self.tracked_vehicles = {
            tid: track for tid, track in updated_tracks.items() 
            if current_time - track['last_seen'] < 2.5  # Increased from 1 to 2.5 seconds
    }

        
    def check_line_crossings_directional(self):
        ...
        for track_id, track in list(self.tracked_vehicles.items()):
            ...
            # Require at least 3 points for more stable detection
            if len(track['path']) < 3:
                continue
            ...
            # Check if vehicle crossed the line (signs are different)
            if cross_product_current * cross_product_prev < 0:
                ...
                if dist_to_line < threshold:
                    ...
                    # Determine direction based on cross product sign
                    # Positive cross product = UP/LEFT direction
                    # Negative cross product = DOWN/RIGHT direction
    ...
    def update_count_labels(self):
        """Update the directional count labels in the GUI"""
        # Total counts
        self.root.after(0, lambda: self.total_up_label.config(text=f"Total UP: {self.total_count_up}"))
        self.root.after(0, lambda: self.total_down_label.config(text=f"Total DOWN: {self.total_count_down}"))
        
        # Individual vehicle counts
        self.root.after(0, lambda: self.car_up_label.config(text=f"↑{self.vehicle_count_up['car']}"))
        self.root.after(0, lambda: self.car_down_label.config(text=f"↓{self.vehicle_count_down['car']}"))
        
        self.root.after(0, lambda: self.moto_up_label.config(text=f"↑{self.vehicle_count_up['motorcycle']}"))
        self.root.after(0, lambda: self.moto_down_label.config(text=f"↓{self.vehicle_count_down['motorcycle']}"))
        
        self.root.after(0, lambda: self.bus_up_label.config(text=f"↑{self.vehicle_count_up['bus']}"))
        self.root.after(0, lambda: self.bus_down_label.config(text=f"↓{self.vehicle_count_down['bus']}"))
        
        self.root.after(0, lambda: self.truck_up_label.config(text=f"↑{self.vehicle_count_up['truck']}"))
        self.root.after(0, lambda: self.truck_down_label.config(text=f"↓{self.vehicle_count_down['truck']}"))
    ...
    def end_line(self, event):
        ...
        if self.drawing_line and hasattr(self, 'line_start_canvas'):
            ...
            if math.sqrt((p2[0] - p1[0])**2 + (p2[1] - p1[1])**2) > 10:
                # Replace the old line with new one (single line only)
                self.counting_line = [p1, p2]
                self.line_drawn = True
                self.line_draw_enabled = False  # Disable after drawing one line
                self.draw_line_button.config(text="Draw Line", state='normal')
                self.line_status.config(text="Line: 1 drawn")
                self.instructions.config(text="Counting line ready! Start detection to count vehicles with direction.")
                self.update_debug_info()
            else:
